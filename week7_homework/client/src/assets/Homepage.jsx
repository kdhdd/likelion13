...
import { getProducts } from "../api";                              // ✅ API 호출용
...
export default function Homepage() {
  const [sortType, setSortType] = useState("high"); // high / low
  const [products, setProducts] = useState([]);      // ★ 서버 데이터 저장
  ...
  useEffect(() => {
    (async () => {
      try {
        // shoes · clothes 중 원하는 카테고리 문자열 넘겨주세요
        const data = await getProducts("shoes");     // ★ 서버 GET 호출
        setProducts(data);
      } catch (err) {
